...
class Version:
    ...
    def increment(self, part: Literal["MAJOR", "MINOR", "PATCH", "BUILD"]) -> None:
        """Increment major, minor, patch, or build"""
        if part == "MAJOR":
            ...
            raise ValueError(
                f"invalid part {part!r}: use MAJOR, MINOR, PATCH, or BUILD"
            )
    ...
    def _next_build(self) -> str:
        ...
        pre = (
            "-" + ".".join(str(c) for c in self.pre_release) if self.pre_release else ""
        )
